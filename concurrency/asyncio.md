...
4. 信号驱动I/O
  - 回调, 非阻塞, I/O事件发生时, 向程序发送信号, I/O操作在用户空间完成
  - 执行I/O操作的函数会立即完成, 因此不会保留执行I/O操作的上下文环境
6. epoll
  - I/O多路复用的改进版本, 统称为基于I/O事件的I/O模型
  - 程序会在执行I/O操作的地方阻塞, 因此保留了执行I/O操作的上下文环境
...
基于信号的回调
  - 特点:
    ...
    - 所有调用都是非阻塞的, 执行I/O操作的函数会立即完成, 因此不会保留执行I/O操作的上下文环境
    - 只有异步应用程序底层的事件循环 **能依靠基础的中断、线程、轮询和后台进程等**,
    确保多个并发请求能取得进展并最终完成, 这样才能使用回调
    - 事件循环获得响应后, 会回过头来调用我们指定的回调
  - 附注:
    - Node.js 不支持使用 JavaScript 编写的用户级线程,
    - 但是在背后却借助 libeio 库使用 C 语言实现了线程池, 以此提供基于回调的文件 API
    - 因为从 2014 年起, 大多数操作系统都不提供稳定且便携的异步文件处理 API 了
    - Node 提供了异步文件系统 API
  - 缺点:
    - 执行分成多步的异步任务时丢失上下文
    - 以及缺少处理错误所需的上下文
...
基于epoll的协程
  ...
  - 所有I/O操所都是异步阻塞的, 保留了执行I/O操作的上下文环境, 能避免可怕的“回调地狱”
